...
| `region` | string | 예 | 소재 지역(`LCTN_SC_NM`). NEIS가 값을 주지 않으면 빈 문자열 |
| `schoolType` | string | 예 | 학교 종류(`SCHUL_KND_SC_NM`). NEIS가 값을 주지 않으면 빈 문자열 |

식별에 필요한 `officeCode`, `schoolCode`, `name`이 없는 항목은 제외한다.
`region`과 `schoolType`은 응답에 항상 포함하되 NEIS가 값을 제공하지 않으면 빈
문자열로 채우고 값을 추정하지 않는다.
...
날짜는 `Asia/Seoul` 기준 캘린더 날짜로 해석하고 UTC 타임스탬프로 다루지 않는다.
미래 날짜 조회는 허용하되 해당 기간의 급식 데이터가 없으면 빈 목록으로 처리한다.
...
통제된 테스트 환경(정상 응답, 단일 페이지)을 기준으로 한다.

- NEIS 연결 타임아웃 약 3초, 응답 타임아웃 약 10초를 기본값으로 하고 환경
  변수로 조정할 수 있다.
- 안전한 조회의 재시도는 최대 2회, 지수 백오프를 적용하며 총 요청 처리 시간에
  상한을 둔다.
- 단일 페이지 조회의 내부 API 응답은 상위 API 정상 응답 시 대체로 1초 이내를
  목표로 한다.
- 프론트엔드는 요청 시작 직후 로딩 상태를 표시하고 실패 시 무한 로딩을 남기지
  않는다.

### 관측성

- 요청마다 상관관계 ID를 부여하고 작업 이름, 소요 시간, 결과, 정제된 상위 API
  상태를 구조적 로그로 남긴다.
- 로그와 예외 메시지에서 API 키(`Key`)와 인증 쿼리를 마스킹한다.
- 상위 API 지연, 타임아웃, 요청 제한, 오류 건수 등 운영 지표를 노출할 수 있다.
- 백엔드는 준비 상태(readiness)와 활성 상태(liveness)를 구분해 헬스 체크로
  제공한다.

### CORS 및 라우팅

- 프론트엔드와 백엔드는 동일 출처 리버스 프록시 또는 명시적 CORS 중 하나로
  통신한다.
- CORS를 사용하면 허용 출처를 `FRONTEND_ORIGIN`으로 명시하고, 필요한 메서드와
  헤더만 허용하며 자격 증명은 요구하지 않는다. 프로덕션에서 와일드카드 출처를
  허용하지 않는다.
...
- 개발자 환경 설정은 `.env`로 주입하되 Compose 파일이나 이미지 레이어에 비밀
  값을 하드코딩하지 않는다.
- 포트, 페이지 크기, 타임아웃, 최대 조회 일수, 프론트엔드 API 기본 경로는 안전한
  기본값을 문서화하고 환경 변수로 재정의한다.
...
- `src/openapi.json`의 OpenAPI 린트·검증을 통과해야 한다.
- 프론트엔드 통합, 백엔드 단위·통합 테스트가 필수로 실행되고 통과해야 한다.
- Compose 기반 E2E 스모크 테스트와 백엔드 헬스 체크가 통과해야 한다.
- 테스트가 실제 NEIS 네트워크에 접근하면 실패로 처리해 결정성을 보장한다.
- 백엔드는 모든 변환과 오류 매핑 분기를 테스트로 덮는다.
