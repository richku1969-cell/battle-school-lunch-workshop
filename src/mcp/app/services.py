...
from app.errors import (
    ...
    no_meals_found_error,
    no_schools_found_error,
)
...
class SchoolService:
    ...
    async def search(self, query: str) -> SchoolSearchResponse:
        ...
        if not schools:
            return SchoolSearchResponse(items=[], total=0, hasMore=False, message=no_schools_found_error().message)
...
class MealService:
    ...
    async def list_meals(self, office_code: str, school_code: str, from_date: date, to_date: date) -> MealResponse:
        ...
        if not items:
            return MealResponse(items=[], message=no_meals_found_error().message)
